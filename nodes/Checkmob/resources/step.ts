...
export const description: INodeProperties[] = [
	{
		...
		options: [
			{ name: 'List', value: 'list', description: 'List steps', action: 'List steps' },
		],
		default: 'list',
	},
	{
	...
	},
	{
		...
		displayOptions: { show: { resource: ['step'], operation: ['list'], returnAll: [false] } },
...
export async function execute(
	...
	if (operation === 'list') {
		...
		const items = await apiRequestAllItems.call(
			this,
			{ url: `${baseUrl}/v2/etapas/list`, headers: authHeaders, body: reqBody, returnAll, limit },
			this.getNode(),
		);
