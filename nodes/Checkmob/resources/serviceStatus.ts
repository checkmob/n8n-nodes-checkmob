...
export const description: INodeProperties[] = [
	...
	},
	{
		...
		displayOptions: { show: { resource: ['serviceStatus'], operation: ['list'], returnAll: [false] } },
...
export async function execute(
	...
	if (operation === 'list') {
		...
		const items = await apiRequestAllItems.call(
			this,
			{ url: `${baseUrl}/v2/status-servico/list`, headers: authHeaders, body: reqBody, returnAll, limit },
			this.getNode(),
		);
