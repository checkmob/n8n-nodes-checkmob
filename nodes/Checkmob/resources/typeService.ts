...
export const description: INodeProperties[] = [
	{
		...
		options: [
			{ name: 'List', value: 'list', description: 'List service types', action: 'List service types' },
		],
		default: 'list',
	},
	{
	...
	},
	{
		...
		displayOptions: { show: { resource: ['typeService'], operation: ['list'], returnAll: [false] } },
...
export async function execute(
	...
	if (operation === 'list') {
		...
		const items = await apiRequestAllItems.call(
			this,
			{ url: `${baseUrl}/v2/tipos-servico/list`, headers: authHeaders, body: reqBody, returnAll, limit },
			this.getNode(),
		);
