...
export const description: INodeProperties[] = [
	{
		...
		options: [
			{ name: 'List', value: 'list', description: 'List market sectors', action: 'List market sectors' },
		],
		default: 'list',
	},
	{
	...
	},
	{
		...
		displayOptions: { show: { resource: ['marketSector'], operation: ['list'], returnAll: [false] } },
...
export async function execute(
	...
	if (operation === 'list') {
		...
		const items = await apiRequestAllItems.call(
			this,
			{ url: `${baseUrl}/v2/setores-mercado/list`, headers: authHeaders, body: reqBody, returnAll, limit },
			this.getNode(),
		);
