...
export const description: INodeProperties[] = [
	{
		...
		options: [
			{ name: 'List', value: 'list', description: 'List objectives', action: 'List objectives' },
		],
		default: 'list',
	},
	{
	...
	},
	{
		...
		displayOptions: { show: { resource: ['objective'], operation: ['list'], returnAll: [false] } },
...
export async function execute(
	...
	if (operation === 'list') {
		...
		const items = await apiRequestAllItems.call(
			this,
			{ url: `${baseUrl}/v2/objetivos/list`, headers: authHeaders, body: reqBody, returnAll, limit },
			this.getNode(),
		);
